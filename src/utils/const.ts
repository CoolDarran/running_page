// Constants
const MAPBOX_TOKEN =
  'pk.eyJ1IjoiY29vbGRhcnJhbiIsImEiOiJjbG9zZGdoejcxNGhzMmlvNjkxazRucTJuIn0.lU1DaA-PPiQ0PIBrZbkXdQ';
...
const CHINESE_INFO_MESSAGE = (yearLength: number, year: string): string => {
  ...
  return `运动 ${yearLength} 年了，下面列表展示的是 ${yearStr} 的轨迹数据`;
};
...
const CHINESE_LOCATION_INFO_MESSAGE_SECOND = '不要停下来，不要停下浪的脚步';
...
const TOTAL_CALORIES_TITLE = IS_CHINESE ? '总卡路里' : 'Total Calories';
...
const WORKOUT_TIILE = IS_CHINESE ? '运动' : 'Workout';
...
export const MAP_TILE_STYLE_DARK = 'camouflage';
